...
const arena = createMatrix(20, 40); // Larger arena
let nextPiece = createPiece('T');

let currentMode = 'mode2'; // 初期設定は操作二

// モードの切り替えボタンを追加
const modeButton = document.createElement('button');
modeButton.innerText = 'モード切り替え';
modeButton.onclick = () => {
    currentMode = currentMode === 'mode1' ? 'mode2' : 'mode1';
    alert('現在のモード: ' + (currentMode === 'mode1' ? '操作一' : '操作二'));
};
document.body.appendChild(modeButton);

document.addEventListener('keydown', event => {
    if (currentMode === 'mode1') {
        handleMode1Keydown(event);
    } else {
        handleMode2Keydown(event);
    }
});
...
function handleMode1Keydown(event) {
    ...
    if (event.keyCode === 37) { // 左矢印キーで移動
        ...
        playerRotate(1);
    } else if (event.keyCode === 32) { // スペースキーでハイパードロップ
        playerHardDrop();
    } else if (event.keyCode === 13) { // エンターキーで停止
        togglePause();
    }
}
